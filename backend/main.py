        ...
        async for message in websocket:
            ...
            if action == "join":
                ...
                history_message = json.dumps(
                    {"action": "load_history", "messages": chat_history}
                )
                ...
                join_message = json.dumps(
                    {
                        ...
                        "message": f"{user} đã tham gia phòng!",
                    }
                )
                ...
                for conn in connected_users[room][:]:
                    if conn != websocket:  # Không gửi cho người vừa tham gia
                        try:
                            ...
                            connected_users[room].remove(
                                conn
                            )  # Loại bỏ kết nối đã bị đóng
                ...
                broadcast_message = json.dumps(
                    {"action": "message", "user": user, "message": msg}
                )
...
# Chạy server WebSocket trên tất cả giao diện mạng
# Sử dụng biến môi trường PORT để thiết lập cổng
